 ...
 * ('ymd' | 'dmy' | 'mdy' — src/lib/settings.ts 참고). referenceDate는 "미래 날짜만 채택"
 * 판단 기준일 — 생략하면 오늘(`todayStr()`), selfcheck에서는 오늘 날짜에 테스트 결과가
 * 좌우되지 않도록 고정값을 넘긴다. */
...
각 후보는 다음 순서로 판단해서 해석한다:

1. **연도 위치**: `a`와 `c` 중 하나가 4자리면 그게 연도(위치가 구조적으로 명확 — 설정과 무관). 둘 다 2자리면(진짜 애매한 경우) 설정값 `dateOcrOrder`로 정한다 — `'ymd'`면 `a`가 연도, `'dmy'`/`'mdy'`면 `c`가 연도.
2. **연도가 맨 앞이면**(국내 표준) 나머지 두 묶음은 항상 `월-일` 순서 — 설정값과 무관. 국내 인쇄에서 "년-일-월" 순서는 실제로 쓰이지 않기 때문.
3. **연도가 맨 뒤면**, 나머지 두 묶음 `(r0, r1)`(원래 순서) 중 하나가 13 이상이면 그건 월이 될 수 없으므로 **그 쪽이 일, 나머지가 월**로 구조적으로 확정된다(설정값과 무관). 예: `(25, 09)` → 25는 월이 될 수 없음 → 일=25, 월=09.
4. 위 3번에서도 못 정해지면(`r0`, `r1` 둘 다 12 이하라 진짜 애매함) 그제서야 설정값을 쓴다 — `'dmy'`면 `r0`=일, `r1`=월. 그 외(`'ymd'`/`'mdy'`)는 `r0`=월, `r1`=일.

예: 후보가 `(09, 10, 2026)`이고 설정이 `'mdy'`면 → `c`(2026, 4자리)가 연도, 나머지 `(09, 10)`은 둘 다 12 이하라 애매 → 설정(`mdy`, `dmy`가 아님)에 따라 월=09, 일=10 → `2026-09-10`.
...
**3. 여러 후보가 있을 때** — 제조일자+유통기한이 함께 찍히는 경우가 흔하므로, 기준일(`referenceDate`, 기본값은 오늘) 이상인 후보만 남기고 그중 가장 늦은 날짜를 채택한다. 남는 후보가 하나도 없으면(사진에 유효기간이 안 보이는 경우) `null`.
...
모든 테스트는 오늘 날짜에 결과가 좌우되지 않도록 `referenceDate`를 `'2026-01-01'`로 고정해서 호출한다.

`date-ocr.selfcheck.ts`:
1. `"유통기한 2026.09.10까지"` (연도 4자리, 앞) → `"2026-09-10"` (설정값 무관)
2. `"EXP 25.09.2026"` (연도 4자리, 뒤, 25는 월이 될 수 없어 구조적으로 일-월 확정) → `"2026-09-25"` (설정값 무관)
3. `"09.10.2026"` (연도 4자리, 뒤, 09·10 둘 다 월/일 가능해 진짜 애매) + `dateOcrOrder: 'mdy'` → `"2026-09-10"`
4. 제조일자+유통기한이 함께 찍힌 텍스트(예: `"제조 2025.06.01 유통기한 2026.09.10"`) → 기준일(2026-01-01)보다 앞선 제조일자는 버려지고 `"2026-09-10"` 채택
